-- Final robust version of the undo_last_match function
...
BEGIN
    ...
    IF p_match_id IS NOT NULL THEN
        SELECT id, winner_id, loser_id, created_at::date
        INTO v_match_id, v_winner_id, v_loser_id, v_match_date
        FROM public.matches
        WHERE id = p_match_id;
    ELSE
        SELECT id, winner_id, loser_id, created_at::date
    ...
    -- 2. Security: Check if the day is already closed
    ...
    -- 4. Recalculate stats for BOTH players
    ...
    FOREACH v_pid IN ARRAY v_player_ids LOOP
        ...
        -- We use both winner_id and loser_id to find all relevant matches
        FOR v_m IN (
        ...
        -- Update daily_stats for the specific date of the match
